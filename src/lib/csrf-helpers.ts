...
export function getCsrfTokenFromCookie(): string | null {
  ...
  const cookie = document.cookie
    .split(';')
    .map(v => v.trim())
    .find(v => v.startsWith(`${CSRF_COOKIE_NAME}=`));
...
export function initializeCsrfFetch(): void {
  ...
  const existing = (window as any).__csrfFetchInitialized;
  if (existing) return;
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (needsCsrfHeader(request.method)) {
      ...
      if (csrfToken) {
        ...
        return nativeFetch(request, { headers });
      }
    }
    return nativeFetch(request);
  };
  (window as any).__csrfFetchInitialized = true;
}
